...
export async function getNutrition(foodQuery, foodQuantity) {

    // ✅ FIX 1: Try Indian food database FIRST, not OpenFoodFacts
    return getIndianFoodNutrition(foodQuery, foodQuantity);
}

function getIndianFoodNutrition(query, quat ) {
    const q = query.toLowerCase().trim();
    const m = quat.toLowerCase().trim().match(/^([\d.]+)\s*(.*)$/);

    // Extract quantity from END of string
    // const quantityMatch = m.match(/\s+(\d+\.?\d*)\s*(g|kg|ml|l|pieces?|pcs|cups?)?$/); // instead of this we have direct quantity field now buddy

    let quantity = m ? parseFloat(m[1]) : 0; // Number: 2.5
    let unit     = m ? m[2].trim()       : ''; // String: "kg"
